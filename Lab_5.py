...
POPULATION_SIZE = 100  # кількість індивідуумів у популяції
MAX_GENERATIONS = 200  # максимальна кількість поколінь

P_CROSSOVER = 0.9  # ймовірність схрещування
P_MUTATION = 0.1  # ймовірність мутації індивідуума
...
def animate(i):
    ...
    ax.text(LIMIT_VALUE_DOWN, LIMIT_VALUE_DOWN, 40000, "Покоління:" + str(generationCounter))
    ...
    print(
        f"Покоління {generationCounter}: Функція пристосованості = {minFitness}, Середнє пристосованості = {meanFitness}")

    best_index = fitnessValues.index(min(fitnessValues))
    print("Найкращий індивідуум = ", *population[best_index], "\n")
...
plt.ylabel('Мін/середня/максимальна пристосовність')

plt.title("Знаходження глобального мінімуму функціЇ ")
